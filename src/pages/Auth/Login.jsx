...
const Login = () => {
  ...
  const { setAuthToken } = useContext(AdminTechZoneContext);

  const onSubmit = async (data) => {
    try {
      setIsLoading(true);
      const response = await apis.apiLoginAdmin({
        email: data.username,
    ...
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 to-gray-800 p-4 sm:p-6">
      <section className="bg-white p-6 sm:p-8 md:p-10 rounded-2xl shadow-2xl w-full sm:w-11/12 md:w-3/4 lg:w-1/2 xl:w-1/3 transform transition-all active:scale-100 sm:hover:scale-105 duration-300">
        <div className="text-center mb-6 sm:mb-8">
          <h1 className="text-2xl sm:text-3xl font-extrabold text-teal-600 mb-2">
            Admin Panel
          </h1>
          <h2 className="text-lg sm:text-xl font-semibold text-gray-700">
            Đăng Nhập
          </h2>
          <div className="border-b-2 border-teal-500 w-16 mx-auto mt-4"></div>
        </div>
        ...
        >
          <div>
            ...
            {errors.username && (
              <p
                className="text-red-500 text-xs sm:text-sm mt-1 animate-pulse"
                role="alert"
              >
          ...
          <div>
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-gray-700 focus:outline-none focus:text-gray-700 transition-colors cursor-pointer"
