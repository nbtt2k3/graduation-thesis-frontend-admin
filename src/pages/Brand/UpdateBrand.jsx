...
import { UpdateBrandValidation } from "../../validations/brand.validation";
import { useParams } from "react-router-dom";
import { toast } from "react-hot-toast";
import * as apis from "../../apis";
import { LoadingSpinner, ConfirmExitModal } from "../../components";
...
const UpdateBrand = () => {
  ...
  const {
    ...
    resolver: yupResolver(UpdateBrandValidation),
    mode: "onChange",
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await apis.apiGetBrand(brandId);
        const brand = response.brand;
        setValue("name", brand.name || "", { shouldValidate: true });
        setValue("isActive", brand.isActive ?? true, { shouldValidate: true });
        if (brand.logoUrl) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await apis.apiUpdateBrand(brandId, formData);
      navigate("/products/brand-management", {
        state: { brandUpdated: response?.msg },
      });
    } catch (error) {
      toast.error(
        error?.msg || "Không thể cập nhật thương hiệu. Vui lòng thử lại."
      );
    } finally {
      setIsLoading(false);
    }
