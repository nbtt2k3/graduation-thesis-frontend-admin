...
import { CreateBrandValidation } from "../../validations/brand.validation";
import { toast } from "react-hot-toast";
import * as apis from "../../apis";
import { ConfirmExitModal } from "../../components";
...
const CreateBrand = () => {
  const {
    ...
    resolver: yupResolver(CreateBrandValidation),
    mode: "onChange",
  });
  ...
  const { navigate } = useContext(AdminTechZoneContext);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await apis.apiCreateBrand(formData);
      navigate("/products/brand-management", {
        state: { brandCreated: response?.msg },
      });
    } catch (error) {
      if (error?.msg) {
        toast.error(
          error.msg || "Không thể tạo thương hiệu. Vui lòng thử lại."
        );
      }
    } finally {
      setIsLoading(false);
    ...
    <div className="p-3 sm:p-6 bg-white h-auto">
      ...
      <div className="bg-white rounded-sm shadow-md">
        <div className="bg-[#00D5BE] text-white p-4 rounded-t-sm">
          <h2 className="font-semibold">THÊM THƯƠNG HIỆU</h2>
        </div>
        <div className="p-6">
