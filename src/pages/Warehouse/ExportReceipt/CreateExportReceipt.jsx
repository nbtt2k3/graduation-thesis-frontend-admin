...
const CreateExportReceipt = () => {
  ...
  const onSubmit = async (data) => {
    // Form-level validation for items array
    ...
    try {
      ...
      const response = await apis.apiCreateExportReceipt({
      ...
      navigate("/warehouse/export-receipt-management", {
        state: { exportReceiptCreated: response?.msg },
      });
    } catch (error) {
      if (error?.msg) {
        toast.error(
          error.msg || "Không thể tạo phiếu xuất kho. Vui lòng thử lại."
        );
      }
    } finally {
      setLoading(false);
    ...
    <div className="p-3 sm:p-6 bg-white h-auto">
      ...
      <div className="bg-white rounded-sm shadow-md">
        <div className="bg-[#00D5BE] text-white p-4 rounded-t-sm">
          <h2 className="text-lg font-semibold">THÊM PHIẾU XUẤT KHO</h2>
        </div>
        <div className="p-6">
          <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    >
                      <CustomSelect
                        ...
                        options={productItems
                          ...
                          .map((product) => ({
                            value: product._id,
                            label: `${product.name} (${product.attributes
            ...
            <div className="flex justify-end space-x-3 mt-8 pt-6 border-t border-gray-200">
              <button
                ...
                disabled={loading}
                aria-label="Hủy tạo phiếu xuất kho"
              >
                Hủy
              </button>
              <button
