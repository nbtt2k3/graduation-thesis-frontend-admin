...
// Define allowed payment methods to avoid hardcoding
...
const UpdateImportReceipt = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [
          ...
          importReceiptId
            ? apis.apiGetImportReceipt(importReceiptId)
            : Promise.resolve({}),
        ]);
        ...
        if (importReceiptId && receiptResponse.importReceipt) {
          const { importReceipt } = receiptResponse;

          if (["approved", "canceled"].includes(importReceipt.status)) {
            toast.error(
              `Phiếu nhập kho đang ở trạng thái ${
                importReceipt.status === "approved" ? "đã duyệt" : "đã hủy"
          ...
          reset({
            supplierId: importReceipt.supplierId?.trim() || "",
            branchId: importReceipt.branchId?.trim() || "",
            items:
              importReceipt.items &&
              Array.isArray(importReceipt.items) &&
              importReceipt.items.length > 0
                ? importReceipt.items.map((item) => ({
                    productItemId: item.productItemId?.trim() || "",
                    quantity: item.quantity
                      ? formatNumber(item.quantity.toString())
                      : "",
                    purchasePrice: item.purchasePrice
                      ? formatNumber(item.purchasePrice.toString())
                      : "",
                  }))
                : [{ productItemId: "", quantity: "", purchasePrice: "" }],
            note: importReceipt.note?.trim() || "",
            paymentMethod: importReceipt.paymentMethod?.trim() || "",
          });
        }
      } catch (error) {
        if (error?.msg) {
    ...
    fetchData();
  }, [importReceiptId, reset, navigate]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await apis.apiUpdateImportReceipt(importReceiptId, {
      ...
      navigate("/warehouse/import-receipt-management", {
        state: {
          importReceiptUpdated: response?.msg,
        },
      });
    } catch (error) {
      if (error?.msg) {
        toast.error(
          error.msg || "Không thể cập nhật phiếu nhập kho. Vui lòng thử lại."
        );
      }
    } finally {
      setIsLoading(false);
    ...
    <div className="p-3 sm:p-6 bg-white h-auto">
      ...
      <div className="bg-white rounded-sm shadow-md">
        <div className="bg-[#00D5BE] text-white p-4 rounded-t-sm">
          <h2 className="text-lg font-semibold">CẬP NHẬT PHIẾU NHẬP KHO</h2>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="flex justify-end space-x-3 mt-8 pt-6 border-t border-gray-200">
              <button
                ...
                disabled={isLoading}
                aria-label="Hủy cập nhật phiếu nhập kho"
              >
                Hủy
              </button>
              <button
                ...
                disabled={isLoading}
                aria-label="Cập nhật phiếu nhập kho"
              >
                {isLoading ? (
                  <>
