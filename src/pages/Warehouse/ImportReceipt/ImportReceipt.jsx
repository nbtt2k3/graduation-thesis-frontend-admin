...
import { CustomSkeletonImportReceipt, CustomSelect } from "../../../components";
...
const ImportReceipt = () => {
  ...
  const [appliedSupplierFilter, setAppliedSupplierFilter] = useState("");
  ...
  const getAllImportReceipts = async (page = 1) => {
    try {
      setLoading(true);
      const query = { page };
      if (appliedCodeFilter) query.code = appliedCodeFilter;
      if (appliedStatusFilter) query.status = appliedStatusFilter;
      if (appliedSupplierFilter) query.supplierId = appliedSupplierFilter;

      const response = await apis.apiGetAllImportReceipts(query);

      setImportReceipts(response.importReceiptList || []);
      ...
      if (page === 1 && response.importReceiptList) {
        setItemsPerPage(response.importReceiptList.length || 5);
      }
    } catch (error) {
      if (error?.msg) {
  ...
  const fetchSuppliers = async () => {
    try {
      const response = await apis.apiGetAllSuppliers({
        limit: 0,
        isActive: true,
      });
  ...
  useEffect(() => {
    getAllImportReceipts(currentPage);
  }, [
    currentPage,
    appliedCodeFilter,
    appliedStatusFilter,
    appliedSupplierFilter,
  ]);

  useEffect(() => {
    if (location.state?.importReceiptCreated) {
      toast.success(location.state.importReceiptCreated);
      getAllImportReceipts(currentPage);
      navigate(location.pathname, { replace: true, state: {} });
    } else if (location.state?.importReceiptUpdated) {
      toast.success(location.state.importReceiptUpdated);
      getAllImportReceipts(currentPage);
      navigate(location.pathname, { replace: true, state: {} });
    }
  ...
  const handleApproveReceipt = async (receiptId, receiptStatus) => {
    try {
      if (receiptStatus !== "pending") {
        toast.error(
          "Chỉ có thể phê duyệt phiếu nhập kho ở trạng thái Đang chờ"
        );
        return;
      }

      const response = await apis.apiApproveImportReceipt(receiptId);

      setImportReceipts((prev) =>
        prev.map((receipt) =>
      ...
      toast.success(response?.msg || "Phê duyệt phiếu nhập kho thành công");
    } catch (error) {
      if (error?.msg) {
        toast.error(
          error.msg || "Lỗi khi phê duyệt phiếu nhập kho. Vui lòng thử lại."
        );
      }
    }
  };
  ...
  const handleCancelReceipt = async (receiptId, receiptStatus) => {
    try {
      if (receiptStatus !== "pending") {
        toast.error("Chỉ có thể hủy phiếu nhập kho ở trạng thái Đang chờ");
        return;
      }

      const response = await apis.apiCancelImportReceipt(receiptId);
      ...
      setPendingReceipts((prev) => prev - 1);
      toast.success(response?.msg || "Hủy phiếu nhập kho thành công");
    } catch (error) {
      if (error?.msg) {
        toast.error(
          error.msg || "Lỗi khi hủy phiếu nhập kho. Vui lòng thử lại."
        );
      }
    }
  };
  ...
  const handleShowDetails = async (receipt) => {
    try {
      const response = await apis.apiGetImportReceipt(receipt._id);

      setSelectedReceipt(response.importReceipt);
    } catch (error) {
      if (error?.msg) {
        toast.error(error.msg || "Không thể tải dữ liệu. Vui lòng thử lại.");
      } else {
  ...
  const handleOpenConfirmModal = (action, receiptId, receiptStatus) => {
    if (receiptStatus !== "pending") {
      toast.error(
        ...
        } phiếu nhập kho ở trạng thái Đang chờ`
      );
      return;
    }
  ...
  if (loading) {
    return <CustomSkeletonImportReceipt />;
  }
    ...
    <div className="p-3 sm:p-6 bg-white h-auto">
          ...
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md mx-auto transform transition-all duration-300 ease-in-out scale-100">
            ...
            <div className="p-6">
              <p className="text-sm text-gray-500 mb-6">
                ...
                {confirmAction === "approve" ? "phê duyệt" : "hủy"} phiếu nhập
                kho này?
              </p>
              ...
              <div className="relative z-10 flex justify-between items-start">
                <div>
                  ...
                  <p className="text-blue-100 text-sm opacity-90">
                    Thông tin chi tiết về phiếu nhập kho
                  </p>
                </div>
                <button
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-900 mt-1 font-medium">
                          {suppliers.find(
                            (s) => s._id === selectedReceipt.supplierId
                          )?.name || "Không xác định"}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-900 mt-1 font-bold text-lg">
                          {selectedReceipt.totalAmount.toLocaleString("vi-VN")}{" "}
                          VNĐ
                        </p>
                      </div>
                    </div>
              ...
              <div className="flex justify-end space-x-3">
                ...
                {selectedReceipt.status === "pending" && (
                  <Link
                    ...
                    aria-label="Chỉnh sửa phiếu nhập kho"
                  >
      ...
      <div className="mb-4 sm:mb-6">
        <div className="flex space-x-1 rounded-lg p-1">
          <Link
            ...
            aria-label="Thêm phiếu nhập kho mới"
          >
      ...
      <div className="bg-white rounded-sm shadow-md">
        <div className="bg-[#00D5BE] text-white p-3 rounded-t-sm flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-2 sm:space-y-0">
          <h2 className="text-base font-semibold">QUẢN LÝ NHẬP KHO</h2>
        </div>
            ...
            <tbody className="divide-y divide-gray-200">
              {importReceipts.map((receipt, index) => (
                <tr key={receipt._id} className="hover:bg-gray-100">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-900">
                    {suppliers.find((s) => s._id === receipt.supplierId)
                      ?.name || "Không xác định"}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-900">
                    ...
                    <div className="flex space-x-2">
                      {receipt.status === "pending" && (
                        <>
                          <Link
                            to={`edit/${receipt._id}`}
                            className="px-3 py-1 text-xs flex items-center rounded transition-colors bg-blue-500 text-white hover:bg-blue-600 cursor-pointer"
                            aria-label="Sửa phiếu nhập kho"
                          >
                          ...
                          <button
                            ...
                            aria-label="Phê duyệt phiếu nhập kho"
                          >
                ...
                <div className="flex flex-col space-y-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1 text-sm text-gray-600">
                      ...
                      <p>
                        Tổng tiền: {receipt.totalAmount.toLocaleString("vi-VN")}{" "}
                        VNĐ
                      </p>
                    </div>
                    <div className="mt-1">
        ...
        {importReceipts.length === 0 && (
          <div className="text-center py-8 text-gray-500">
            {appliedCodeFilter || appliedStatusFilter || appliedSupplierFilter
              ? "Không có phiếu nhập kho nào phù hợp với điều kiện lọc"
