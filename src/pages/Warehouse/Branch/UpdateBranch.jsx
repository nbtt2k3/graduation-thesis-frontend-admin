...
const UpdateBranch = () => {
  ...
  useEffect(() => {
    if (branchId) {
      const fetchBranch = async () => {
        try {
          const response = await apis.apiGetBranch(branchId);
          const branch = response.branch;

          const provinceData = dvhcvn.data.find(
            (p) => p.name === branch.province
          );
          ...
          setValue("name", branch.name, { shouldValidate: true });
          setValue("addressLine", branch.addressLine, { shouldValidate: true });
          setValue("phone", branch.phone, { shouldValidate: true });
          setValue("province", provinceId, { shouldValidate: true });
          setValue("district", districtId, { shouldValidate: !!provinceId });
          setValue("ward", wardId, { shouldValidate: !!districtId });
          setValue("isActive", branch.isActive, { shouldValidate: true });
          setValue("latitude", branch.location.coordinates[1].toString(), {
            shouldValidate: true,
          });
          setValue("longitude", branch.location.coordinates[0].toString(), {
            shouldValidate: true,
          });
  ...
  const handleSearchLocation = async () => {
    ...
    if (coordinates) {
      ...
      clearErrors(["latitude", "longitude", "form"]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await apis.apiUpdateBranch(branchId, formData);
      navigate("/warehouse/branch-management", {
        state: { branchUpdated: response?.msg },
      });
    } catch (error) {
      if (error?.msg) {
        toast.error(
          error.msg || "Không thể cập nhật chi nhánh. Vui lòng thử lại."
        );
      }
    } finally {
      setIsLoading(false);
    ...
    <div className="p-3 sm:p-6 bg-white h-auto">
      ...
      <div className="bg-white rounded-sm shadow-md">
        <div className="bg-[#00D5BE] text-white p-4 rounded-t-sm">
          <h2 className="text-lg font-semibold">CẬP NHẬT CHI NHÁNH</h2>
        </div>
        <div className="p-6">
          <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div>
                  <label className="inline-flex items-center">
                    <input
                      ...
                      disabled={isLoading}
                      onChange={() => clearErrors(["isActive", "form"])}
                    />
            ...
            <div className="flex justify-end space-x-3 mt-8 pt-6 border-t border-gray-200">
              <button
                ...
                disabled={isLoading}
                aria-label="Hủy cập nhật chi nhánh"
              >
                Hủy
              </button>
              <button
