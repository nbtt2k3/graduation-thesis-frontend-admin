...
const CreateVoucher = () => {
  ...
  const onSubmit = async (data) => {
    try {
      setIsLoading(true);
      const response = await apis.apiCreateVoucher({
        name: data.name.trim(),
      ...
      navigate("/promotions/voucher-management", {
        state: { voucherCreated: response?.msg },
      });
    } catch (error) {
      if (error?.msg) {
    ...
    <div className="p-3 sm:p-6 bg-white h-auto">
      ...
      <div className="bg-white rounded-sm shadow-md">
        <div className="bg-[#00D5BE] text-white p-4 rounded-t-sm">
          <h2 className="text-lg font-semibold">THÊM VOUCHER</h2>
        </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    {...register("applyTo", {
                      ...
                      validate: (value) =>
                        VOUCHER_APPLY_TO.includes(value) ||
