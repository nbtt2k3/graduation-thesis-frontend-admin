...
const CreateCoupon = () => {
  ...
  const handleTypeSelect = async (type) => {
    ...
    setIsTypeDropdownOpen(false);
    const currentValue = getValues("value").replace(/[^0-9]/g, "");
    if (currentValue) {
    ...
    if (type === "fixed") {
      ...
      const currentMaxValue = getValues("maxValue").replace(/[^0-9]/g, "");
  ...
  const onSubmit = async (data) => {
    try {
      setIsLoading(true);
      const response = await apis.apiCreateCoupon({
        name: data.name,
        description: data.description,
        type: data.type,
        value: Number(data.value.replace(/[^0-9]/g, "")) || 0,
        minValue: Number(data.minValue.replace(/[^0-9]/g, "")) || 0,
        maxValue: Number(data.maxValue.replace(/[^0-9]/g, "")) || 0,
      ...
      navigate("/promotions/coupon-management", {
        state: { couponCreated: response?.msg },
      });
    } catch (error) {
      if (error?.msg) {
        toast.error(error?.msg || "Không thể tạo voucher. Vui lòng thử lại.");
      }
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="p-3 sm:p-6 bg-white h-auto">
      ...
      <div className="bg-white rounded-sm shadow-md">
        <div className="bg-[#00D5BE] text-white p-4 rounded-t-sm">
          <h2 className="text-lg font-semibold">THÊM COUPON</h2>
        </div>
                  ...
                  <input
                    ...
                    {...register("value", {
                      ...
                      validate: {
                        ...
                        maxNumber: (val) =>
                          Number(val.replace(/[^0-9]/g, "")) <= MAX_NUMBER ||
                          "Giá trị giảm không được vượt quá 16 chữ số",
                        maxLength: (val) =>
                          val.replace(/[^0-9]/g, "").length <= 16 ||
                      ...
                      validate: {
                        ...
                        matchFixedValue: (val) =>
                          ...
                          Number(val.replace(/[^0-9]/g, "")) ===
                            Number(getValues("value").replace(/[^0-9]/g, "")) ||
