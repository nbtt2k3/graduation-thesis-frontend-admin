...
const UpdateCoupon = () => {
  ...
  useEffect(() => {
    const fetchCoupon = async () => {
      try {
        setIsLoading(true);
        const response = await apis.apiGetCoupon(couponId);
        const coupon = response.coupon;
        const formattedValidFrom = formatLocalDate(new Date(coupon.validFrom));
        const formattedValidTo = formatLocalDate(new Date(coupon.validTo));
        setSelectedType(coupon.type);
        setSelectedApplyTo(coupon.applyTo);
        ...
        if (validToDate <= today) {
          ...
          toast.error("Coupon đã hết hạn, không thể chỉnh sửa.");
          navigate("/promotions/coupon-management");
          return;
        }
      } catch (error) {
        if (error?.msg) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const response = await apis.apiUpdateCoupon(couponId, payload);
      navigate("/promotions/coupon-management", {
        state: { couponUpdated: response?.msg },
      });
    } catch (error) {
      if (error?.msg) {
        toast.error(
          error?.msg || "Không thể cập nhật voucher. Vui lòng thử lại."
        );
      }
    } finally {
      setIsLoading(false);
    ...
    <div className="p-3 sm:p-6 bg-white h-auto">
      ...
      <div className="bg-white rounded-sm shadow-md">
        <div className="bg-[#00D5BE] text-white p-4 rounded-t-sm">
          <h2 className="text-lg font-semibold">CẬP NHẬT COUPON</h2>
        </div>
        <div className="p-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    {...register("name", {
                      ...
                      minLength: {
                        value: 2,
                        message: "Tên coupon phải có ít nhất 2 ký tự",
                      },
                      maxLength: {
                        value: 100,
                        message: "Tên coupon không được vượt quá 100 ký tự",
                      },
                      validate: {
                        notOnlyWhitespace: (value) =>
                          value.trim().length > 0 ||
                          "Tên coupon không được chỉ chứa khoảng trắng",
                      },
                    })}
                  ...
                  <input
                    ...
                    {...register("type", {
                      ...
                      validate: (value) =>
                        ...
                        `Loại giảm giá chỉ được là: ${COUPON_TYPES.join(", ")}`,
                    })}
                  />
                        ...
                        validDate: (val) => {
                          ...
                          if (originalDate <= today) {
                            return (
                              ...
                              "Coupon đã bắt đầu, không thể chỉnh sửa ngày bắt đầu."
                            );
                          }
                          return (
                            selectedDate > today ||
                      ...
                      validate: {
                        ...
                        notChangeIfExpired: (val) => {
                          ...
                          if (originalDate <= today) {
                            return (
                              val === originalValidTo ||
                              "Coupon đã hết hạn, không thể chỉnh sửa ngày kết thúc."
                            );
                          }
                          return true;
                        },
